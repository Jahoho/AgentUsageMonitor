...
- `OverviewView`: active-agent summary, expandable weekly recap, all-provider activity aggregation, and concise provider source health.
...
Official history is deliberately separated from the provider snapshot lifecycle. After a dashboard refresh has published its current provider snapshots and menu-bar status, `QuotaProjectionService` receives the raw snapshots returned by `ProviderMonitorService`, before any permitted last-known fallback merge. It accepts Codex only in the first version and requires all of the following:
...
The history read path terminates in a separate `QuotaProjection` model used only by the compact projection card. Quota observations never become `ProviderSnapshot` values, and no provider adapter, current-quota bar, menu-bar status or dashboard fallback path reads them. History therefore cannot populate current Official bars or make a failed provider appear healthy. A Keychain or history-store failure leaves current provider health unchanged and makes only the projection unavailable.

## Quota Projection

`QuotaProjectionAnalyzer` is a pure Core calculation. It matches samples only within the same provider, opaque account scope, quota-window id and Official reset cycle. It removes an isolated one-sample spike and starts a new trend when remaining capacity rises by more than two percentage points, so a reset or capacity adjustment is not interpreted as consumption.

Every projection requires at least five samples and sufficient density. Windows whose observed reset lead exceeds 24 hours use up to 36 hours of history and require 6 hours of coverage; a gap above 4 hours starts a new recent-pace segment while the endpoints remain in the broad elapsed-time trend. Shorter windows use up to 3 hours, require 30 minutes and permit gaps no larger than 45 minutes. Both require at least 15% of the expected five-minute sampling density.

The analyzer applies monotonic normalization and a Huber-weighted linear fit. Short windows blend that fit with recent pace. Long windows anchor their baseline to consumption across elapsed calendar time, then give the latest continuous pace less weight as forecast distance grows. Changes spanning a sampling gap are distributed across the crossed activity buckets rather than assigned to one endpoint bucket. A 70/30 historical holdout, fit uncertainty, discrete quota-step timing and activity-bucket pace variation produce a remaining-at-reset range.
...
The Overview-only weekly recap reuses the same protected scalar quota history and does not add a second persistence path. Codex windows are identified by the official duration when it is available, so a weekly-only source exposed as `primary` is still labeled `codex-weekly`. Older history that labeled a long primary window as `codex-session` is normalized only in memory when its reset lead proves it cannot be the five-hour window; the JSONL file is not rewritten.

`WeeklySubscriptionReviewAnalyzer` segments weekly observations when remaining capacity materially refills, while reset-time adjustments without a refill stay in the same cycle. A pre-reset capacity correction may create a segment for analysis, but it is not eligible for recap until that segment's own reset has actually passed. The analyzer reviews the most recent completed cycle rather than repeating current quota. A recap requires capture near both cycle boundaries and at least 15% of the expected five-minute samples. The UI labels this local retrospective as `Observed`, never `Official` or `Estimated`.

The recap reports the final observed headroom near reset and whether the cycle entered the five-percent low-headroom zone. Rhythm attributes quota decreases only across sample intervals no longer than four hours, then describes whether meaningful use was quiet, concentrated, steady, or mixed and identifies the weekday with the largest trustworthy drop. Longer gaps remain unattributed instead of being assigned to an endpoint day.

A personal baseline compares the completed cycle with the median of up to four earlier trustworthy cycles and appears only when at least three exist. The plan-fit row evaluates up to four recent completed cycles after at least three are available: three cycles ending with at least 35% headroom produce an ample-headroom pattern, while two cycles reaching 5% or less produce a frequent-pressure pattern. These are descriptive history summaries, not upgrade or downgrade recommendations. Missing coverage remains explicit; the first version does not extrapolate a whole cycle, send notifications, or affect the menu-bar icon.
