...
| Codex official quota samples | Support local quota projection and subscription review analysis | Scalar samples in `quota-observations-v1.jsonl`; account identity is replaced with a keyed opaque scope before storage, and projections are computed locally rather than persisted |
...
Quota projections are computed on the Mac from those scalar samples and the current Official quota. Projection outputs are held in memory, are not sent to the maintainer or a third party, and are never used as a fallback when current Official quota is unavailable.
